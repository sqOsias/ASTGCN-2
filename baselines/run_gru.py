...
def train(args):
    ...
    model = GRUPredictor(input_size=1, hidden_size=args.hidden_size, output_size=args.horizon).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), 'baselines/results/best_gru.pth')
    ...
    model.load_state_dict(torch.load('baselines/results/best_gru.pth'))
    model.eval()
    preds = []
    trues = []
    with torch.no_grad():
    ...
    results_path = os.path.join(results_dir, 'gru_results.npz')
    np.savez_compressed(
        results_path,
        prediction=preds,
        ground_truth=trues,
