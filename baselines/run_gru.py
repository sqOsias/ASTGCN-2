"""
GRU Baseline — Training Script
Saves: checkpoint (.pth), training log (.csv), normalization stats (.json)
Usage: python -m baselines.run_gru [--epochs 100 ...]
"""
import sys
import os
import json
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        if avg_val < best_val_loss:
            ...
            ckpt_path = os.path.join(results_dir, f'best_gru_h{args.horizon}.pth')
            torch.save({
    ...
    # ---- save training log ----
    log_csv = os.path.join(results_dir, f'gru_h{args.horizon}_training_log.csv')
    ...
    meta_path = os.path.join(results_dir, f'gru_h{args.horizon}_meta.json')
    meta = {
        'model': 'GRU',
        'hidden_size': args.hidden_size,
    ...
    print(f"  Checkpoint : {os.path.join(results_dir, f'best_gru_h{args.horizon}.pth')}")
