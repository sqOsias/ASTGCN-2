...
def test(args):
    ...
    h = args.horizon
    meta_path = os.path.join(results_dir, f'lstm_h{h}_meta.json')
    if not os.path.exists(meta_path):
        raise FileNotFoundError(
            f"{meta_path} not found. Please run training first:\n"
            f"  python -m baselines.run_lstm --horizon {h}"
        )
    with open(meta_path) as f:
        meta = json.load(f)

    print("=" * 60)
    print("LSTM Test — loading checkpoint")
    ...
    # ---- load model ----
    ckpt_path = os.path.join(results_dir, f'best_lstm_h{h}.pth')
    ...
    metrics_csv = os.path.join(results_dir, f'lstm_h{h}_test_metrics.csv')
    with open(metrics_csv, 'w', newline='') as f:
        w = csv.writer(f)
        w.writerow(['model', 'sensor_id', 'seq_len', 'horizon', 'MAE', 'RMSE', 'MAPE(%)'])
        w.writerow(['LSTM', meta['sensor_id'], meta['seq_len'], meta['horizon'], mae, rmse, mape])
    ...
    # ---- save predictions ----
    npz_path = os.path.join(results_dir, f'lstm_h{h}_results.npz')
    np.savez_compressed(
        npz_path,
        prediction=preds,
        ground_truth=trues,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Test LSTM baseline (uses saved checkpoint)')
