...
import csv
import numpy as np
import torch  # MXNet:ndarray interop → PyTorch:tensor interop
...
def evaluate(net, test_loader, true_value, num_of_vertices, sw, epoch, horizons=None):
    '''
    ...
    prediction_flat = (prediction.transpose((0, 2, 1))
    ...
    for i in horizons:
        ...
        mae = mean_absolute_error(true_value[:, : i * num_of_vertices],
                                  prediction_flat[:, : i * num_of_vertices])
        rmse = mean_squared_error(true_value[:, : i * num_of_vertices],
                                  prediction_flat[:, : i * num_of_vertices]) ** 0.5
        mape = masked_mape_np(true_value[:, : i * num_of_vertices],
                              prediction_flat[:, : i * num_of_vertices], 0)
