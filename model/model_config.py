...
def get_backbones(config_filename, adj_filename, ctx):
    ...
    cheb_polynomials = [torch.tensor(i, dtype=torch.float32, device=ctx) for i in cheb_polynomial(L_tilde, K)]  # MXNet:nd.array(ctx=ctx) → PyTorch:torch.tensor(device=ctx)
